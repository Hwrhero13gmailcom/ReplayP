...
static Replay::Events::CutInfo CutInfoFrom(NoteCutInfo const& c) {
    ...
    ci.beforeCutRating  = c.beforeCutRating;
    ci.afterCutRating   = c.afterCutRating;
    return ci;
}
...
static Replay::Events::NoteInfo NoteInfoFrom(NoteController* note, Replay::Events::NoteInfo::Type type) {
    ...
    ni.scoringType  = (short) data->scoringType;
    ni.lineIndex    = (short) data->lineIndex;
    ni.lineLayer    = (short) data->noteLineLayer;
    ni.colorType    = (short) data->colorType;
    ni.cutDirection = (short) data->cutDirection;
    ni.eventType    = type;
    return ni;
}
...
static void WriteVector3(std::ofstream& out, Vector3 v) {
    Write(out, v.x);
    Write(out, v.y);
    Write(out, v.z);
}

static void WriteQuaternion(std::ofstream& out, Quaternion q) {
    Write(out, q.x);
    Write(out, q.y);
    Write(out, q.z);
    Write(out, q.w);
...
std::string Recorder::SaveReplay() {
    // Save into the same folder Replay mod reads from: BeatLeader replays dir
